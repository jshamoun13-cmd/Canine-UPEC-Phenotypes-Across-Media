...
out_dir <- file.path(getwd(), "Growth_PCA_Outputs_0to10h")
...
run_medium_pipeline <- function(df, medium_name, time_points_keep, out_dir) {
  ...
  df_t <- df_t %>% dplyr::slice(1:n_keep)
  ...
  t_file <- file.path(out_dir, paste0(medium_name, "_t_0to10h.xlsx"))
  write_xlsx(df_t, t_file)
  message("Saved reshaped table: ", t_file)
  ...
  hist_file <- file.path(
    out_dir,
    paste0(medium_name, "_sigma_histogram_0to10h.png")
  )
  ...
  metric_cols <- pca_gc_med %>% dplyr::select(k:sigma)
  
  pca_med <- prcomp(metric_cols, center = TRUE, scale. = TRUE)
  ...
  message("Top 5 by σ for ", medium_name, ":")
  print(top_sigma)
  ...
  pca_png <- file.path(
    out_dir,
    paste0(medium_name, "_PCA_growth_metrics_0to10h.png")
  )
  ...
  out_xlsx <- file.path(
    out_dir,
    paste0(medium_name, "_growth_PCA_outputs_0to10h.xlsx")
  )
...
res_LB  <- run_medium_pipeline(LB, "LB", time_points_10, out_dir)
res_AUM <- run_medium_pipeline(AUM, "AUM", time_points_10, out_dir)
res_M9  <- run_medium_pipeline(M9, "M9", time_points_10, out_dir)
...
p_LB <- ggplot(scores_LB, aes(PC1, PC2)) +
  ...
  xlim(x_lim) +
  ylim(y_lim) +
  ggtitle("PCA of growth metrics — LB (0–10 h)") +
  theme_bw()
...
p_AUM <- ggplot(scores_AUM, aes(PC1, PC2)) +
  ...
  xlim(x_lim) +
  ylim(y_lim) +
  ggtitle("PCA of growth metrics — AUM (0–10 h)") +
  theme_bw()
...
p_M9 <- ggplot(scores_M9, aes(PC1, PC2)) +
  ...
  xlim(x_lim) +
  ylim(y_lim) +
  ggtitle("PCA of growth metrics — M9 (0–10 h)") +
  theme_bw()
...
ggsave(
  file.path(out_dir, "LB_PCA_growth_metrics_0to10h_fixedaxes.png"),
  p_LB,
  width = 8,
  height = 6,
  dpi = 300
...
ggsave(
  file.path(out_dir, "AUM_PCA_growth_metrics_0to10h_fixedaxes.png"),
  p_AUM,
  width = 8,
  height = 6,
  dpi = 300
...
ggsave(
  file.path(out_dir, "M9_PCA_growth_metrics_0to10h_fixedaxes.png"),
  p_M9,
  width = 8,
  height = 6,
  dpi = 300
...
stats_label <- paste0(
  "PERMANOVA (k–σ): R² = ", round(permanova_res$R2[1], 3),
  ", F = ", round(permanova_res$F[1], 2), ", ",
  fmt_p(permanova_res$`Pr(>F)`[1]), "\n",
  "Dispersion (betadisper): F = ", round(bd_anova$`F value`[1], 2), ", ",
  fmt_p(bd_anova$`Pr(>F)`[1]), "\n",
  "Kruskal–Wallis PC1: χ² = ", round(kw_PC1$statistic, 1), ", ",
  fmt_p(kw_PC1$p.value), "\n",
  "Kruskal–Wallis PC2: χ² = ", round(kw_PC2$statistic, 1), ", ",
...
ggsave(
  filename = file.path(
    out_dir,
    "$Figure 2b Growthcurver Metrics at 10h by Medium.png"
  ),
  plot = p_global,
  width = 9,
  height = 6.5,
